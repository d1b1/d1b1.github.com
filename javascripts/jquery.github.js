 ...
 * a specific user using the GitHub.com JSON feeds (http://delicious.com/help/feeds/),
 * and without need for any server-side component.
 *
 * @param String user The Github.com user who's bookmarks you want to load.
 ...
 * @option String tag Available for type 'posts'. Indicates the tag or tags of which all returned posts should be a member.
 ...
 * @type jQuery
 * @name github.com
 * @cat Plugins/github.com
 *
 */
$.fn.github = function(user,options,tOptions,cbFnc){
	$.defineTag('script');
	var opts = $.extend({'user':user},$.github.opts,options),
		$self = this,
		fn = cbFnc || jQuery.github.parsers[opts.type],
		url = 'http://github.com/api/v2/json/repos/show/d1b1/?',
		rOpts = $.extend({raw:'true',callback:name(fn)},$.github.types[opts.type],tOptions);
	...
	function name( callback ) {
		var i = $.github.callbacks.length;

		$.github.callbacks[i] = function( json ) {
			delete $.github.callbacks[i];
			$self.each( function() { callback.apply(this,[json,opts]); } );
		};

		return 'jQuery.github.callbacks['+i+']';
	};
};
...
$.github = {
	...
	parsers : {
		posts : function(data,opts){
			...
			$.github.add(this,$[opts.wrapTag]({},lis),opts);
		},
		tags : function(data,opts){
			var lis = [];
			$.each(data,function(tName,tCount){
				var fIcon, oSpan;
				var a_opts = {href:'http://github.com/'+opts.user+'/'+tName};
				if(opts.popopen) a_opts['target'] = '_blank';
				lis[lis.length] = $[opts.itemTag]({},
		...
		network : function(data,opts){
			...
			$.each(data,function(i,name){
				var a_opts = {href:'http://github.com/'+name};
				if(opts.popopen) a_opts['target'] = '_blank';
				lis[lis.length] = $[opts.itemTag]({},
					$.A(a_opts, name)
				);
