...
One point that the Development Seed author notes, 'no entry point to hack', is a nice bi-product of the github SSH security model. Unless a hacker can grab a private SSH key (and has the passphrase) the github servers are the only point of entry. If a hacker defaces the github files, git would allow a simple 'git push origin master' to clear the offending content. 
